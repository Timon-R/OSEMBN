...
def get_types_from_tuple(index: list, param: str, config: Dict) -> Tuple:
    ...
    for name, element in zip(names, index):
        ...
        elif this_type == 'int' or this_type == 'int64':
...
def apply_yearly_interploted_values(
    ...
    """
    try: # replaces the values in the rows of the DataFrame model_params[name] where the index is between tuple(index + [first_year]) and tuple(index + [end_year]) with the values in interpolated_values.
        # Convert interp_values to an iterator
        ...
        logger.info(f"Error raised in parameter {name} by index {[tuple(index + [first_year]), tuple(index + [end_year])]}")
        logger.info(f"Error raised in parameter {name} by index {index}")
        logger.info(model_params[name])
        raise ValueError(ex)
    return model_params
...
if __name__ == "__main__":
    ...
    #for debugging:
    # with open("modelruns/0/model_0/sample_0.txt", 'r') as csv_file:
    #     sample = list(csv.DictReader(csv_file))
    
    # _, ending = os.path.splitext("config/otoole.yaml")
    # with open("config/otoole.yaml", "r") as f:
    #     user_config = _read_file(f, ending)

    # main("input_data/WP1_NetZero/data", "results/0/model_0//data", sample, user_config)
