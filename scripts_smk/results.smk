...
rule extract_results: #write results which are to analyse into seperate csv files
    input: 
        csvs=get_input,
    ...
    log: "log/extract_results/extract_scenarion_{scenario}_{result_file}.log"
    output: expand("results/{{scenario}}/{{result_file}}.{ext}", ext=config['filetype'])
    conda: "../envs/otoole_env.yaml"
    script: "gsa/extract_results.py"
...
rule calculate_SA_objective:
    ...
    shell: "python scripts_smk/gsa/calculate_SA_results.py {params.parameters} {input.sample} {input.results} {output[0]} {params.result_type} {params.scaled}"
...
rule create_heatmap:
    ...
    conda: "../envs/heatmap_env.yaml"
    shell: 
        "python scripts_smk/gsa/create_heatmap.py {params.parameters} {input.sample} {input.results} {output} {params.scaled}"
...
rule plot_interactions:
    ...
    conda: "../envs/sample_env.yaml"
    shell:
        "python scripts_smk/gsa/plot_interactions.py {params.parameters} {input.sample} {input.results} {output}"
